...
func writerClose() {
	...
	data := []byte("Testing with closed writer!")
	...
	fmt.Println("Written n", n)
	err = writer.Close()
	if err != nil {
		panic(err)
	}
	fmt.Println("Compressed", outBuffer.Bytes())
	reader := brotli.NewDecompressionReader(outBuffer)
	_, err = io.Copy(decBuffer, reader)
	fmt.Println(string(decBuffer.Bytes()))

}

func stream() {
	data := []byte("I can stream, still not good enough!")
	...
	fmt.Println("Compressed", outBuffer.Bytes())
	readerD := brotli.NewDecompressionReader(outBuffer)
	_, err = io.Copy(decBuffer, readerD)
	fmt.Println(string(decBuffer.Bytes()))
}

func concat() {
	data := []byte("This could work, each writer concats bytes to the buffer and performs the stream compression that is afterword read by the reader")
...
func transport() {
	...
	data := []byte("This is the closest to stream compression")

	inBuffer := bytes.NewBuffer(nil)
	linkBuffer := bytes.NewBuffer(nil)
	// outBuffer := bytes.NewBuffer(nil)

	brotliCompress := brotli.NewMultiCompressionReader(inBuffer, options)
	n, err := inBuffer.Write(data)
	if err != nil {
		panic(err)
	}
	fmt.Println(n)
	_, err = io.Copy(linkBuffer, brotliCompress)
	if err != nil {
		panic(err)
	}

	readerD := brotli.NewDecompressionReader(linkBuffer)
	b := make([]byte, 100)
	n, err = readerD.Read(b)
	if err != nil {
		if err != io.EOF {
			panic(err)
		}
	...
	// nc, err = io.Copy(outBuffer, readerD)
	// if err != nil {
	// 	panic(err)
	// }
	// fmt.Println(nc, string(outBuffer.Bytes()))
}
